...
public class DragAdapter extends BaseAdapter {
    ...
    private List<DragItemEntiy> mAddedItems;
    private List<DragItemEntiy> mNotAddedItems;
    ...
    public DragAdapter(Context context, ArrayList<DragItemEntiy> added, ArrayList<DragItemEntiy> notAdded, String centerTip) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        DragItemEntiy itemObj = getLDragItem(position);
        if(getItemViewType(position)==TYPE_ITEM){
    ...
    private DragItemEntiy getLDragItem(int position){
        if (position==mAddedItems.size()){
            return null;
        }
    ...
    public boolean exchange(int scrPos, int desPos){
        ...
        if(scrPos!=desPos){
            if(scrPos<count){
                DragItemEntiy srcObj = mAddedItems.get(scrPos);
                if (desPos<count){
                    //change in add rang
                    DragItemEntiy desObj = mAddedItems.get(desPos);
                    mAddedItems.set(scrPos, desObj);
                    mAddedItems.set(desPos, srcObj);
                }else {
                    //change from add to notAdd
                    desPos = desPos-mAddedItems.size()-1;
                    DragItemEntiy desObj = mNotAddedItems.get(desPos);
                    mAddedItems.set(scrPos, desObj);
                ...
                DragItemEntiy srcObj = mNotAddedItems.get(scrPos);
                if (desPos<count){
